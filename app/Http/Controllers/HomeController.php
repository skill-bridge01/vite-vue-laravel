    ...
    public function index():array
    {
        ...
        if($slugId){
            ...
            if($publishedData->isNotEmpty()){
                $homes = CmsPage::whereIn('id', $pageIds)->get();
                $seoItems=Seo::where('model_type', 'App\Models\CmsPage')->whereIn('model_id', $pageIds)->get();
                $seoMap = $seoItems->keyBy('model_id');

                foreach ($homes as $home) {
                    $home->seo = $seoMap[$home->id] ?? null;
                }
                return ['home' => $homes,'success' => 1, 'message' => 'Home fetched successfully.'];
            } else {
                return ['success' => 0, 'message' => 'There are no published home'];
            }
    ...
    public function about():array
    {
        ...
        if($slugId){
            ...
            if($publishedData->isNotEmpty()){
                ...
                return ['about' => $abouts,'success' => 1, 'message' => 'HomeAbout fetched successfully.'];
            } else {
                return ['success' => 0, 'message' => 'There are no published HomeAbout'];
            }
